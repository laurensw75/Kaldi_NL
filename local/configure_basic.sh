...
if [ ! -d models/NL ]; then
	...
	mkdir -p $modelpack
	[ ! -e $modelpack/Models_Starterpack.tar.gz ] && wget --no-check-certificate -P $modelpack http://nlspraak.ewi.utwente.nl/open-source-spraakherkenning-NL/Models_Starterpack.tar.gz
...
if [ ! -e models/Patch1 ]; then
	...
	[ ! -e $modelpack/Models_Patch1.tar.gz ] && wget --no-check-certificate -P $modelpack http://nlspraak.ewi.utwente.nl/open-source-spraakherkenning-NL/Models_Patch1.tar.gz
